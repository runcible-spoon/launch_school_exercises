...
A straightforward way to search the phone book would be to go through the phone book one name at a time, checking whether the current name is the one you're trying to find.

This may be a simple and easy way to search, but it's not very efficient.
In the worst case scenario, it could mean having to search through every single name in the book, and some phone books could be over 1000 pages.
A linear search such as this can take quite a long time.

A binary search is a much more efficient alternative.
This algorithm lets you cut the search area in half on each iteration by discarding the half that you know your search term doesn't exist in.
...
def binary_search(lst, target, original_list=None):
    if original_list is None:
        original_list = lst

    elif target == lst[len(lst) // 2]:
        return original_list.index(target)
    if len(lst) == 1:
        return -1
    elif target > lst[len(lst) // 2]:
        return binary_search(lst[len(lst) // 2:], target, original_list)
    else:
        return binary_search(lst[:len(lst) // 2], target, original_list)
