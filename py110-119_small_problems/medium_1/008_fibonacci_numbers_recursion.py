...
This sequence can also be computed using a recursive function.
...
A recursive function has three primary qualities:
    ...
    This is a condition that tells the function to stop recursing and begin the process of returning to the first call to the function.
    This is often the simplest case - the condition for which you already know the answer.
    In sum_recursive, the base case occurs when n == 1.
    ...
    For instance, in sum_recursive(3), we call sum_recursive(2), which is closer to the base case.
    Likewise, sum_recursive(2) subsequently calls sum_recursive(1), which is the base case.
...
If you study this set of rules, you can see that the algorithm is defined recursively:
    ...
    In this case, both F(n - 1) and F(n - 2) are closer to the base case than F(n).
...
This exercise borders on the Advanced level of exercises, so do not be discouraged if you are unable to solve it on your own.
Recursion is tricky, and even experienced developers can have difficulty dealing with it.
However, the practice is worthwhile; recursion plays a huge part in many algorithms.
'''
